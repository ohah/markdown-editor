...
첫 구현 목표는 "가장 얇게 실제로 편집되는 경로"를 세우는 것이다. 화려한 기능이 아니라, **앞으로 지켜야 할 경계(문서 원천·에디터 facade·IPC·원자적 저장)를 컴파일·테스트 가능한 형태로 고정**하는 것이 목적이다.
...
수동(macOS + 3-OS):
  같은 흐름을 손으로 확인(특히 Linux WebKitGTK에서 CM6 편집/스크롤/폰트)
```
...
- **셸이 위험하지 않다**: 3-OS(특히 리눅스)에서 실제로 편집·저장이 된다.
