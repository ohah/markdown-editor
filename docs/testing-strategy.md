...
- **개발·검증은 macOS 우선(로컬)**: 앱 실행 프로토타입까지의 실제 동작 확인은 맥 환경에서 로컬로 한다. macOS는 tauri-driver E2E가 안 되므로(아래 "E2E의 플랫폼 한계") 앱 동작은 **로컬 수동 실행**으로 확인한다. Windows/Linux 검증은 프로토타입 이후 단계와 자동화 백로그에서 다룬다.
...
- **플랫폼 웹뷰 렌더 차이**(특히 Linux WebKitGTK): 프로토타입 이후 플랫폼 검증 단계에서 스크린샷 수동 확인 + 가능하면 자동 E2E 연결.
...
강제 지점은 CI가 아니라 **로컬 git 훅**이다. `core.hooksPath = .githooks`로 리포에 커밋된 훅을 쓰며, 훅은 `mise run` 태스크를 호출한다.

```text
.githooks/pre-commit   빠른 피드백: mise run lint + mise run fmt-check
.githooks/pre-push     PR 전 필수 게이트: mise run check (fmt-check + lint + fsd-check + test + clippy)
```

- **pre-push가 "PR 올리기 전 반드시 실행"의 실체다.** 린트와 유닛 테스트가 통과하지 않으면 push가 막힌다.
- 훅은 툴체인이 아직 없으면(docs-only 단계) 조용히 건너뛰고, 스캐폴딩 후 자동으로 활성화된다.
- 새 클론에서 훅을 켜려면 `git config core.hooksPath .githooks`(스캐폴딩 후엔 mise/`prepare`가 자동 설정). 명령은 [development-commands.md Git 훅](development-commands.md).
