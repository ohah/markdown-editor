...
마크다운 텍스트
  ...
  --(rehype-sanitize)---->  hast  (XSS 차단, 기본 on)
  --(rehype-* 플러그인)-->  hast  (수식/코드 하이라이트/각주 등)
  --(rehype-stringify)--->  HTML  (리딩 뷰 주입 / 파일 익스포트)
```

- **보안 기본값**: `rehype-sanitize`는 항상 켠다. 신뢰할 수 없는 문서의 raw HTML/스크립트를 실행하지 않는다([project-rules.md 보안](project-rules.md)).
...
- 새 플러그인은 (1) 성능 영향, (2) sanitize 정책과의 상호작용을 [decisions.md](decisions.md) 또는 PR에 기록하고 추가한다.
