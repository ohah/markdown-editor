...
> 명령 인터페이스는 `mise run …`이다(단일 출처 [development-commands.md](development-commands.md)). 아래 표는 각 영역이 어느 태스크·메커니즘으로 검증되는지 표기한다.
...
| 경로 검증 | `mise run test` (cargo, `fs-engine`) | — | 워크스페이스 탈출 경로를 거부한다. |
| 워크스페이스 검색 | `mise run test` (cargo, `workspace-index`) | — | ripgrep식 검색 결과가 정확하다. |
| IPC 클라이언트 | `mise run test` (`packages/ipc` 모킹) | — | 커맨드 인자/반환 타입 계약이 지켜진다. |
| FSD 경계 | `mise run fsd-check` (steiger) | — | 레이어/슬라이스 import 방향 위반이 없다. |
| 린트/포맷 | `mise run lint`, `mise run fmt-check`, `mise run clippy` | — | 코드 스타일 게이트. |
| 컴포넌트/통합 | `mise run test` (RTL + Vitest) | — | 위젯·기능이 렌더/상호작용한다. |
| E2E(열기·편집·저장·전환·검색) | `mise run e2e` (tauri-driver, **Linux/Windows**) | 스크린샷/로그 | 실제 앱에서 핵심 흐름이 왕복한다(백로그, CI 강제 아님). macOS는 로컬 수동. |
| 프론트 빌드 | `mise run build-web` | 번들 | 프로덕션 번들이 성공한다. |
| Tauri 빌드 | `mise run build` | 앱 산출물 | 현재 OS 타깃으로 앱이 빌드된다. |
...
새 테스트 파일을 추가하면 같은 PR에서 mise 태스크(`.mise.toml`)·CI에 연결한다. 자동화가 불가능하면 문서와 PR에 다음을 남긴다: 왜 자동화가 불가능한가, 어떤 수동 검증을 했는가, 나중에 자동화하려면 어떤 경계가 필요한가.
